...
public class WatchlistDAOImpl implements WatchlistDAO {
    ...
    public boolean insert(Watchlist w) {
        String sql = """
            INSERT INTO Watchlist
              (Watchlist_ID, Watchlist_Name, Notes, Added_Date, Stock_ID, User_ID)
            VALUES (?,?,?,?,?,?)
        """;
        try (PreparedStatement ps = DBUtil.getConnection().prepareStatement(sql)) {
            ...
            ps.setInt   (6, w.getUserId());          // field exists in model
    ...
    public boolean delete(Integer id) {
        try (PreparedStatement ps = DBUtil.getConnection()
                .prepareStatement("DELETE FROM Watchlist WHERE Watchlist_ID=?")) {
            ps.setInt(1, id);
            return ps.executeUpdate() > 0;
        } catch (Exception e) { e.printStackTrace(); }
        return false;
